...
- Medium arrays (~96 bytes to ~1 KB) now use a lightweight AVX2 `popcnt256` loop
  rather than the scalar path or Harley-Seal — roughly 1.2–3x faster than scalar
  across that range on a modern AVX2 CPU. Below ~96 bytes the scalar POPCNT path
  is kept (faster there, including on older CPUs), and the AVX2 dispatch switches
  to Harley-Seal at 1 KB. The 96 B and 1 KB thresholds follow the scalar/AVX2 and
  lookup/Harley-Seal crossovers measured across Haswell..Cascadelake in the
  sse-popcount benchmarks, so older x86 CPUs stay on their fastest kernel too.
